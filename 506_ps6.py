...
airport_data = {}
try:
    airport_response = requests.get(baseurl+airport, params =url_parameters) # result is now a response object. (similar to a file object, sort of)
...
def get_airport(strng):
    airport_data = {}
    try:
        airport_response = requests.get(baseurl+strng, params =url_parameters) # result is now a response object. (similar to a file object, sort of)
        airport_data = json.loads(airport_response.text) 
...
def extract_airport_data(strng): 
    airport_data = {}
    airport_response = requests.get(baseurl+strng, params =url_parameters) # result is now a response object. (similar to a file object, sort of)
    airport_data = json.loads(airport_response.text) 
...
for airprt in fav_airports:
    print extract_airport_data(airprt)
...
new_file = open("airport_temps.csv", 'w')
new_file.write ('airport_code, status_reason, current_temp, recent_update\n')
for airport in possible_airports:
    try:
